"""
T042 v2 — Cross-Plane Path Validation
=======================================
Spec source: CYB-25 §7.2 (Cross-Plane Path Validation)
Validates canonical traversal paths confirming semantic correctness
(in addition to structural completeness verified by T041).

Changes vs validate_t042_crossplane.py:
  PATH4 corrected: the original query was
      ATT_CK_Technique -[:AFFECTS_PROCESS]-> Physical_Process
  which does not match the schema.  Per CYB-13 §II, AFFECTS_PROCESS goes
  from OT_System to Physical_Process.  The correct cross-plane path is:
      ATT_CK_Technique -[:TARGETS]-> OT_System -[:AFFECTS_PROCESS]-> Physical_Process
  Confirmed by stuxnet_relationships.yaml:
      T0831 -[:TARGETS]-> OTS-PLC  AND  OTS-PLC -[:AFFECTS_PROCESS]-> PHY-CENT

Run:
    cd CyberKG-CPS
    python scripts/validate_t042_crossplane_v2.py

Prerequisites:
    - T041 v2 must pass first
...
# Path 2: Stuxnet vulnerability chain (T1091 → CVE-2010-2568 → CWE-20 → CAPEC-210)
PATH2 = """
...
# Path 3a: TRITON — Attack → unauthorized Bridge BRG-EWS
PATH3A = """
...
# Path 4: Stuxnet — Technique → OT_System → Physical_Process (corrected from v1)
# v1 incorrectly queried ATT_CK_Technique -[:AFFECTS_PROCESS]-> Physical_Process
# which never matches (AFFECTS_PROCESS: OT_System → Physical_Process per CYB-13 §II).
# Correct path: Technique -[:TARGETS]-> OT_System -[:AFFECTS_PROCESS]-> Physical_Process
PATH4 = """
MATCH (a:Attack {attack_id: 'ATK-STX-001'})
      -[:USES_TECHNIQUE]->(t:ATT_CK_Technique)
      -[:TARGETS]->(s:OT_System)
      -[:AFFECTS_PROCESS]->(p:Physical_Process)
RETURN a.attack_id AS attack, count(DISTINCT t) AS technique_count,
       collect(DISTINCT p.process_id) AS physical_processes
"""
...
def run_validation():
    ...
    with driver.session() as s:
        ...
        # ── Path 4: Stuxnet — Technique → OT_System → Physical_Process ──────
        rows = list(s.run(PATH4))
        if rows and rows[0]["physical_processes"]:
            r = rows[0]
            ok(f"Path 4: Stuxnet {r['technique_count']} technique(s) -> OT_System -> {r['physical_processes']}")
        else:
            err("Path 4: Stuxnet Technique -> OT_System -> Physical_Process chain not found")
    ...
    # Summary
    print("\n" + "="*60)
    print("T042 CROSS-PLANE PATH VALIDATION SUMMARY (v2)")
    print("="*60)
    for msg in passed:
        print(msg)
    for msg in failed:
...
if __name__ == "__main__":
    print("CyberKG-CPS -- T042 v2: Cross-Plane Path Validation")
    print("Spec: CYB-25 §7.2 | Paths: Colonial, Stuxnet vuln chain, TRITON, Stuxnet physical, all IC\n")
    run_validation()
