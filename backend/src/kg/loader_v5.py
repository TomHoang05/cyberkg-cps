"""loader_v5.py — Split-file YAML loader with plane labels + edge provenance.

Extends the 10-Jul loader.py with two improvements carried forward from the
04-Jul-2026 session (loader_v4):

  1. **Plane multi-labels** — every node gets its plane label(s) (e.g., :Cyber,
     :Physical, :AI, :Human) in addition to the entity-type label.  Required for
     plane-filtered queries (CYB-13 §II).

  2. **Relationship provenance** — every edge gets the 6-field provenance block
     (source, source_id_or_url, ingested_at, confidence, evidence_class, license)
     if not already present.  Required per CYB-12 §V provenance spec.

  3. **entity_id synthesis** — sets `entity_id` = primary key value on every node
     (e.g., entity_id = "T1091" for ATT_CK_Technique).  Required by T041 validator.

File loading convention (unchanged from loader.py):
  data/kg_data/shared/instructional_concepts.yaml   → load FIRST
  data/kg_data/shared/network_zones.yaml            → load SECOND
  data/kg_data/{case}/{prefix}_{plane}.yaml × 4     → nodes
  data/kg_data/{case}/{prefix}_relationships.yaml   → edges

Usage (from cyberkg-cps/ root):
    py -3.11 scripts/load_all_cases_v5.py

Changes vs loader.py (10-Jul baseline):
  - PLANE_LABELS dict added
  - _IC_PLANE_MAP dict added
  - _load_nodes: applies plane labels and synthesises entity_id after MERGE
  - _load_relationships: adds _fill_rel_provenance call before MERGE
  - All other behaviour identical (granularity validation, auto_fill_provenance, etc.)
"""
