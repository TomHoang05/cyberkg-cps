"""T055 v2 -- Sprint 3 API unit tests (fixes for TestHealth + TestChain).

Changes vs test_queries.py:
  - TestHealth.test_health_200:
      Mock run().single()["c"] correctly (was iter([...]) which has no .single()).
  - TestChain.test_chain_happy_path_colonial:
      Pass MOCK_CHAIN_ROWS (plain dicts) directly; dict(dict) works, dict(MagicMock) did not.
  - TestChain.test_chain_steps_ordered:
      Same fix as above.

All other tests are unchanged from test_queries.py.

Run: pytest tests/api/test_queries_v2.py -v
...
class TestHealth:
    ...
    Fix vs v1: mock_session.run() must return a MagicMock whose .single()
    returns a dict-like object.  iter([...]) has no .single() method and
    caused the health handler to catch an AttributeError and return
    success=False.
    """

    def test_health_200(self, client, mock_session):
        # Probe: RETURN 1 -- just needs to not raise
        probe_result = MagicMock()

        # Entity count: MATCH (n) RETURN count(n) AS c
        entity_result = MagicMock()
        entity_result.single.return_value = {"c": 104}

        # Relationship count: MATCH ()-[r]->() RETURN count(r) AS c
        rel_result = MagicMock()
        rel_result.single.return_value = {"c": 147}

        mock_session.run.side_effect = [probe_result, entity_result, rel_result]
...
class TestChain:
    ...
    Fix vs v1: dict(MagicMock(keys=lambda s: ...)) raised TypeError because
    Python calls mock.keys() as mock.keys.__call__() with no args, but the
    lambda required one positional arg ('s').  Fix: pass plain dicts directly
    so that dict(existing_dict) works via the standard copy-constructor path.
    """

    def test_chain_happy_path_colonial(self, client, mock_session):
        """TC-SPR3-003: colonial chain has 3 steps, bridge_type=authorized."""
        # Pass plain dicts directly -- dict(d) where d is a dict just copies it
        mock_session.run.return_value = list(MOCK_CHAIN_ROWS)
